...
(function(){try{
 ...
 const sources={
  search:{name:names.search,url:'/Affiliate_Lab/guides/search-traffic/',pace:'Медленный старт',desc:'Подходит, если готов отвечать на уже существующие вопросы и ждать, пока страницы начнут получать показы.',first:'Выбрать одну узкую тему и собрать 5–10 реальных вопросов пользователей.'},
  ...
  community:{name:names.community,url:'/Affiliate_Lab/guides/community-traffic/',pace:'Работает через доверие',desc:'Сильный вариант, если люди уже читают канал или группу и воспринимают рекомендации автора.',first:'Разделить каналы и размещения метками, затем сравнивать их по одной воронке.'},
  social:{name:names.social,url:'/Affiliate_Lab/guides/social-traffic/',pace:'Нужна регулярность',desc:'Подходит для проверки разных подач в ленте, но требует серии публикаций, а не одного удачного поста.',first:'Выбрать один формат и выпустить 10–20 сопоставимых публикаций.'},
  ads:{name:names.ads,url:'/Affiliate_Lab/guides/paid-traffic/',pace:'Быстрые данные, платный риск',desc:'Даёт быструю обратную связь, если площадка разрешает такой трафик и расход ограничен заранее.',first:'Задать бюджет, ожидаемый объём кликов и точку остановки до запуска.'},
  stream:{name:names.stream,url:'/Affiliate_Lab/guides/stream-traffic/',pace:'Сильно зависит от ведущего',desc:'Подходит тем, кто готов работать вживую и может удерживать внимание, а не просто вывести ссылку на экран.',first:'Провести 3–5 эфиров и отдельно записать онлайн, клики и дальнейшие действия.'},
...
(function(){try{
 ...
 const context=from.startsWith('/Affiliate_Lab/')?{url:from,title:known?.title||'Материал Affiliate Lab'}:null;
